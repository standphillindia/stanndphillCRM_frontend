// src/components/invoice/AmcInvoiceFormModal.tsx
...
export default function AmcInvoiceFormModal({
  ...
  const [piOptions, setPiOptions] = useState<AmcInvoicePaymentRow[]>([]);
  const [selectedPiId, setSelectedPiId] = useState<string>("");

  useEffect(() => {
    if (isOpen && invoiceType === "TAX" && amc) {
      getAmcInvoicesWithPayment(amc.id)
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    {
      id: "1",
      description: invoiceType === "PROFORMA" ? "AMC Service Charge" : "AMC Renewal Charge",
      sacCode: "998393",
      quantity: 1,
      unitPrice: 0,
      amount: 0,
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[95vh] overflow-y-auto">
        <div className={`sticky top-0 bg-gradient-to-r ${theme.from} ${theme.to} text-white p-6 flex justify-between items-center`}>
          <h2 className="text-2xl font-bold">
            Create {invoiceType === "PROFORMA" ? "Proforma Invoice" : "Tax Invoice"} — {amc.amcCode}
          </h2>
        ...
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-2 text-sm text-amber-900">
          ℹ️ AMC total contract value: <strong>₹{(amc.amount || 0).toLocaleString()}</strong> — enter only
