// src/components/invoice/ProjectInvoiceFormModal.tsx
// Direct Project-scoped mirror of AmcInvoiceFormModal.tsx — same behaviour,
// just creates invoices against a Project instead of an AMC contract. This
// is what the new Payments -> List -> Project page uses (replaces the old
// Deal-based ProformaInvoiceModal / TaxInvoiceModel).
...
export default function ProjectInvoiceFormModal({
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    {
      id: "1",
      description: invoiceType === "PROFORMA" ? "Certification Service Charge" : "Certification Renewal Charge",
      sacCode: "998393",
      quantity: 1,
      unitPrice: 0,
      amount: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : `Failed to create ${invoiceType === "PROFORMA" ? "PI" : "TI"}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[95vh] overflow-y-auto">
        <div className={`sticky top-0 bg-gradient-to-r ${theme.from} ${theme.to} text-white p-6 flex justify-between items-center`}>
          <h2 className="text-2xl font-bold">
            Create {invoiceType === "PROFORMA" ? "Proforma Invoice" : "Tax Invoice"} — {project.projectName}
          </h2>
        ...
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-2 text-sm text-amber-900">
          ℹ️ Project total value: <strong>₹{(project.amount || 0).toLocaleString()}</strong> — enter only
          the amount you're billing <em>in this specific invoice</em> below (could be the full amount or a partial installment).
        </div>
        ...
        {invoiceType === "TAX" && (
          <div className="bg-blue-50 border-b border-blue-200 px-6 py-4">
            ...
            >
              <option value="">Don't link — standalone TI (no payment tracking)</option>
              {piOptions.map((pi) => (
            ...
            <p className="text-xs text-blue-700 mt-2">
              Linking shows this TI's Received/Pending/Status as a live mirror of the selected PI's actual
              payment — it's the same money, never counted twice. Leave unlinked only for a TI that has no
              PI behind it at all.
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t">
            <button type="submit" disabled={loading}
              ...
              {loading ? "Creating..." : `Create ${invoiceType === "PROFORMA" ? "Proforma Invoice" : "Tax Invoice"}`}
            </button>
